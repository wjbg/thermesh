...
def solve_ht(domain: Domain,
             solver: dict[str, float]) -> tuple[vector, matrix]:
...
class Domain:
    ...
    def timestep(self, dt: float, theta: float = 0.5):
        ...
        if self.check_bc():
            ...
            f = (1-theta)*dt*self.q + dt*theta*q_new  # misses contribution from Q
...
class Mesh:
    ...
    def __init__(self, z: vector, element: LinearElement):
        ...
        if (len(z) - 1) % (element.order) == 0:
            self.nn = len(z)
            self.nodes = z
            self._conn = np.array([np.arange(i, i+1+element.order) for
...
class LinearElement(Element):
    ...
    def K(self, mat: dict[str, float]) -> matrix:
        """Returns element stiffness matrix."""
        K = (mat["k"]/self.length()) * np.array([[1, -1],
                                                 [-1, 1]])
        return K

    def C(self, mat: dict[str, float]):
        """Returns element damping matrix."""
        C = (self.length()*mat["rho"]*mat["cp"]/6) * np.array([[2, 1],
                                                               [1, 2]])
        return C
